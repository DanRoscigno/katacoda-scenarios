...
`grep -A4 filebeat.autodiscover course/filebeat.yml`{{execute HOST2}}
...
docker.elastic.co/beats/filebeat:6.4.0 filebeat -e -strict.perms=false`{{execute HOST2}}
...
`docker inspect nginx | grep -A4 Labels`{{execute HOST2}}
